...
class ReminderPlugin(Plugin):
    metadata = PluginMetadata(
        name="reminder", version="0.1.0", description="Create, cancel, list, and trigger reminders.", tags=["reminder"],
    )
    ...
    def tasks(self) -> list[Task]:
        return [
            _CreateTask(self.service), _CancelTask(self.service),
            _ListTask(self.service), _TriggerTask(self.service),
        ]
...
class _ListTask(Task):
    ...
    def __init__(self, service): self.service = service
    async def run(self, context: TaskContext) -> TaskResult:
        return TaskResult.ok("reminders listed", reminders=[r.__dict__ for r in self.service.list()])
...
class _TriggerTask(Task):
    ...
    def __init__(self, service): self.service = service
    async def run(self, context: TaskContext) -> TaskResult:
        return TaskResult.ok("reminders triggered", reminders=[r.__dict__ for r in self.service.due()])
